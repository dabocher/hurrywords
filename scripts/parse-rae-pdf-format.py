...
def clean_definition(text: str) -> str:
    ...
    # Elimina composiciones precedidas por ■ (son sub-entradas, no definiciones)
    text = re.sub(r'■.*?(?=☐|➤|\Z)', '', text, flags=re.DOTALL)
    # Elimina locuciones precedidas por ➤ (son sub-entradas, no definiciones)
    text = re.sub(r'➤[^ǁ.]*', '', text)
    # Elimina locuciones precedidas por ☐
    text = re.sub(r'☐.*?(?=➤|\Z)', '', text, flags=re.DOTALL)
    # Elimina "U. t. c." (Uso también como) - notas de uso
    text = re.sub(r'\s+U\.\s*t\.\s*c\.\s*[a-z.]*\b', '', text)
...
_GRAM_LOOKAHEAD = (
    r'\((?:De |Del |Cf\.|Etim\.|Quizá |Der\.|Voz )'  # inicio de etimología
    r'|adj\b|adv\b'
...
_ETYM_RE = re.compile(
    r'^\s*\(('
    r'(?:Del?|Cf\.|Etim\.|De\s|Quizá\s|Der\.|Voz\s)[^)]{3,}'
    r')\)\s*\.?\s*',
    re.IGNORECASE,
)
...
def _split_senses_preserving_parens(text: str) -> list[str]:
    """Divide en ǁ pero preserva ǁ dentro de paréntesis (cross-references)."""
    placeholder = '\x01'
    depth = 0
    result = []
    for ch in text:
        if ch == '(':
            ...
            depth = max(0, depth - 1)
            result.append(ch)
        elif ch == 'ǁ' and depth > 0:
            result.append(placeholder)
        else:
            result.append(ch)
    processed = ''.join(result)
    # ǁ separa sensos normales | ⚪ separa con grammar antes del número | ⚫ separa con grammar ANTES (split antes de ⚫)
    blocks = re.split(r'ǁ\s*|⚪.*?(?=\d+\.)|(?<=\.\s)⚫', processed)
    return [b.replace(placeholder, 'ǁ') for b in blocks]
...
def parse_senses(body: str, default_grammar: str | None) -> list[dict]:
    ...
    # Remover composiciones/locuciones (■ marca el inicio de sub-entradas)
    body = body.split('■')[0]

    # Divide en bloques de acepción (preservando ǁ dentro de paréntesis)
    sense_blocks = _split_senses_preserving_parens(body)
    ...
    for i, block in enumerate(sense_blocks):
        ...
        # Gramática propia de esta acepción (override) — antes del número
        # porque ⚫ pone grammar antes del número: "m. y f. 2. ..."
        sense = {'number': i + 1}
        ...
        # Número de acepción (debe estar al inicio del bloque, después de grammar)
        nm = re.match(r'^(\d+)\.\s*', block)
        if nm:
            sense['number'] = int(nm.group(1))
            block = block[nm.end():]
    ...
    for raw in entries:
        ...
        if main_doc:
            ...
            if lemma not in grouped and lemma not in _existing_lemmas:
                ...
                # Fusiona acepciones si la entrada ya existe (homógrafos)
