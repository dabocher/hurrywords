...
3. Ejecuta `./init.sh`. Si falla, paras y reportas.
   Recuerda: `init.sh` instala dependencias él mismo (`npm ci` / `npm
   install`). Si falla ahí, es un problema real (red, registro, conflicto
   de versiones) — repórtalo tal cual, no lo enmascares.
...
> "Investiga cómo se tipa `CounterProps` en `components/Counter.tsx`.
> Escribe tus hallazgos en `progress/research_counter.md`. Tu respuesta a
> mí debe ser solo: `done -> progress/research_counter.md` o un mensaje de
> bloqueo."
...
| Requiere e2e             | 1 implementer (corre Playwright) + 1 reviewer       | Ver `requires_e2e` en feature_list.json |
| Muy compleja             | Divide en sub-tareas y vuelve a aplicar la tabla    |               |

## Qué NO haces

- ❌ Editar archivos en `app/`, `components/`, `lib/`, `convex/` o `tests/`.
- ❌ Marcar features como `done` (eso lo hace el implementer tras revisión).
- ❌ Ejecutar `npm install <paquete>` tú mismo — eso es trabajo del implementer, con justificación.
